...
public class Equi2Rect {
    ...
    static public void extractRectilinear(
            double yaw,
            double pitch,
            double fov,
            ImageBuffer2 imgBuf,
    ...
    private static void math_setLookUp(int width) {
...
//				d1 = 0.000244140625D;
    ...
    private static void math_extractview(
        ...
        math_transform(
                imgBuf,
                imgBuf.getPanoWidth(),
                imgBuf.getPanoHeight(), //pd.length + deltaYHorizonPosition,
                v,
                //hv,
                rectWidth,
                v.length / rectWidth,
...
// 	 */
    ...
    private static void math_transform(
        ...
        int l24 = 0;
        int pd_0[] = imgBuf.getRow(0);
        int pd_1[] = imgBuf.getRow(1);
...
//		if (pitch + vfov_2 > 50D || pitch - vfov_2 < -50D) N_POINTS_INTERP_X = vw/40; 
        if (pitch + vfov_2 > 65D || pitch - vfov_2 < -65D) {
            N_POINTS_INTERP_X = vw / 35;
        }
        if (pitch + vfov_2 > 70D || pitch - vfov_2 < -70D) {
            N_POINTS_INTERP_X = vw / 50;
        }
        if (pitch + vfov_2 > 80D || pitch - vfov_2 < -80D) {
                ...
                for (int x = x_min + 1; x <= x_max;) {
                    ...
                    for (int kk = 0; kk < N_POINTS_INTERP_X_P1; kk++) {
                        ...
                        if (nn) {
                            ...
                            pd_row = imgBuf.getRow(row_index);
                        }
                        if (v_idx == 0) {
                            // draws the pixel
                            xs_org = xs;
                            if (v_idx == 0) {
                                if (nn) {
                                    ...
                                    if ((ys = ys_org) == l24
                                        ...
                                        l24 = ys;
                                        pd_0 = imgBuf.getRow(ys);
                                        pd_1 = imgBuf.getRow(ys + 1);
                                        px00 = pd_0[xs];
                                        px10 = pd_1[xs++];
                                        px01 = pd_0[xs];
                                        px11 = pd_1[xs];
                                        ...
                                        if (ys < 0) {
                                            pd_0 = imgBuf.getRow(0);
                                            l24 = 0;
                                        } else if (ys > miy) {
                                            pd_0 = imgBuf.getRow(miy);
                                            l24 = miy;
                                        } else {
                                            pd_0 = imgBuf.getRow(ys);
                                            l24 = ys;
                                        ...
                                        if (++ys < 0) {
                                            pd_1 = imgBuf.getRow(0);
                                        } else if (ys > miy) {
                                            pd_1 = imgBuf.getRow(miy);
                                        } else {
                                            pd_1 = imgBuf.getRow(ys);
                                        }
                                        if (xs < 0) {
                                            px00 = pd_0[mix];
                                            px10 = pd_1[mix];
                                    ...
                                    if (lanczos2) //v[idx] = lanczos2_interp_pixel( pd, pw, ph - deltaYHorizonPosition, xs_org, ys_org, dx, dy);
                                    {
                                        v[idx] = lanczos2_interp_pixel(imgBuf, pw, ph, xs_org, ys_org, dx, dy);
                                    } else {
